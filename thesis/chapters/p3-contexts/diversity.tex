...
Indeed, it should be noted that several stopping strategies reported maximum levels of~\gls{acr:cg} close to the absolute maximum observed -- \blueboxbold{SS2-NT}, \blueboxbold{SS3-NC}, \blueboxbold{SS4-SAT}, \blueboxbold{SS9-TIME} and \blueboxbold{SS11-COMB} all report good performance. For \blueboxbold{SS5-COMB}, the relatively low mean depths per query at which the greatest level of~\gls{acr:cg} was reached also demonstrates that the combination strategy was particularly robust at detecting a query of poor performance, and subsequently stopping at shallower depths (thus saving the simulated searcher's time). Taking this further, an interesting trend that was initially observed in Figure~\ref{fig:ch8_sim_perf_plots} shows that peak~\gls{acr:cg} is attained at generally shallower mean depths per query under tasks using system \blueboxbold{D} than when compared to system \blueboxbold{ND}.
...
So, what about the raw values? We are interested in max CG. So what do some numbers show?
Considering peak~\gls{acr:cg}, Table~\ref{tbl:ch8_sim_perf} reports...

We should also mention aspectual recall.
Figure 8.x considers the following plots. Shows mean AR.

Taking each strategy in turn...



% - explain figure 7.6. cg vs d/q for performance runs.
% - general trends:
%     - mean cg steadily increases, then begins to tail off at greater mean depths
%     - increase up to a certain point, this is the maximum level of CG.
%     - we see a trend where tasks using system D generally outperform system ND. this is reflected by the lines for D-AS and D-AD being above ND-AS and ND-AD.
%     - demonstrates that the diversification algorithm was working as expected.
%     - see slightly better performance from D-AS than D-AD. however, very minimal.
%
%     - all four interfaces start at very similar across low depths per query, peak, and begin to spread out at the mean d/q increases.
%
% - table (maxCG) reports the values of these peaks.
%     - demonstrate what the peaks are.
%
% - what about aspectual recall?
%     -
