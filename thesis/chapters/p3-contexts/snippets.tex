...
\begin{table}[h!]
    ...
    \begin{center}
        \begin{tabulary}{\textwidth}{L{3.2cm}@{\CSONEHALF}D{2.6cm}@{\CSONEHALF}D{2.6cm}@{\CSONEHALF}D{2.6cm}@{\CSONEHALF}D{2.6cm}@{\CS}}
            & \dbluecell\small\textbf{T0} & \dbluecell\small\textbf{T1} & \dbluecell\small\textbf{T2} & \dbluecell\small\textbf{T4}\\
            
            \RS \lbluecell\small\textbf{SS5-COMB} & \cell\small 3.22 $\pm$ 1.22 & \cell\small 3.22 $\pm$ 1.22 & \cell\small 3.22 $\pm$ 1.22 & \cell\small 3.22 $\pm$ 1.22 \\
            \RS \lbluecell\small\textbf{SS11-COMB} & \cell \small3.22 $\pm$ 1.22 & \cell\small 3.22 $\pm$ 1.22 & \cell\small 3.22 $\pm$ 1.22 & \cell\small 3.22 $\pm$ 1.22 \\
        \end{tabulary}
        \end{center}
    \end{table}

Upon closer examination of Table~\ref{tbl:ch7_sim_perf}, we also find that the other combination strategy \blueboxbold{SS5-COMB} (relating to a combination of both frustration and satisfaction stopping strategies) consistently placed second in performance rankings across the four experimental interfaces trialled, very close behind the mean~\gls{acr:cg} values attained by \blueboxbold{SS11-COMB} (with mean~\gls{acr:cg} values $\pm$ standard deviations for both combination strategies reported in the small table above). Again, this demonstrates that a combination strategy appears to be very effective at eliciting good levels of performance, and suggests that a degree of flexibility is advantageous to searchers. However, these levels of~\gls{acr:cg} were attained over much higher mean depths per query than the other combination strategy counterpart ($14.76$, $16.45$, $15.22$ and $14.58$ for interfaces \blueboxbold{T0}, \blueboxbold{T1}, \blueboxbold{T2} and \blueboxbold{T4} respectively). Over interface \blueboxbold{T0} especially, a high number of non-relevant documents were required to be found ($24$) before stopping.


\todo{======}

- what is the best performing strategy overall?
- table x.x presents the maximum levels of cg attained, the d/q at which this was attained, and the parameter threshold value that was used to attain it. the D/Q value and CG value correspond to the highest point for each interface in the plots in figure x.x.
- the table highlights the best performing strategy yielding the highest level of overall cg over each interface.
- consistently shows that the highest level of cg attained is by SS11, combination strategy.

- what are the highest values for SS11? D/Q?
- say that actually it's very close between a majority of them, and to discern between good performing and poor performing approaches, we ran statistical tests. where the tests did not yield a sig diff, then performance was similar. these are highlighted by the blue shaded cells in the table, and the white cells represent a sig diff -- meaning that the CG was significantly different from the best performing strategy, SS11.

- interesting to note that the other combo strategy SS5 also performs very well.
- so we plotted the performance of the two combination rules. because in the main plots, the line represents the best combination.
- we can see there really is not much difference between varying the parameters -- they all seem to do quite well.

- satisfaction.
- frustration and disgust.
- simple rules do well!
- fixed depth assumption also holds well, although the depths at which the highest cg (SS11 vs. SS1 are much greater, especially for T0). so a searcher has to browse to greater depths on averge to get a similar level of CG. at lower depths per query, searchers will attain much lower d/q -- pull some numbers out to demonstrate the difference.
- time based rules do well. interestingly, the fixed time rule performed better on average than the adaptive time based rule.

- ift doesnt do as well.
- rbp doesnt do as well.

- difference rules are terrible.
...
- then a few lines per stopping strategy.
- how did they perform to the best one?
