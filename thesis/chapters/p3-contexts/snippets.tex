...
In this section, we report on a simulated analysis that takes the thirteen different snippet level stopping strategies described in Chapter~\ref{chap:strategies}

- simulated analysis
- key aim of this is to provide answers to HL-RQ3a and HL-RQ3b, under the context of varying snippet lengths.
- as snippet lengths vary, do different snippet level stopping strategies perform better? do different approaches offer better approximations of searcher behaviour?

\subsection{Methodology}
- as previously discussed, we follow the general simulation methodology we outlined in Section~\ref{chap:csm:method:simulation}.

- stochastic in nature, with the decision makers for snippets and documents (whether i should click on a result summary link, whether i should save a document) determined by a series of probabilities.
- in order to differentiate between the different experimental interfaces, we extracted the interaction probabilities and interaction costs from the log data, discussed in Section~\ref{chap:csm:method:simulation:grounding}.

- for the four interfaces, the interaction probabilities and costs are outlined in Table~\ref{}.

- of particular importance to the work for this particular study are 
