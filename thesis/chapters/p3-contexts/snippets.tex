...
Before discussing our performance \emph{(what-if)} results, we must first determine whether the implemented querying strategy \blueboxbold{QS13} delivered queries of the expected performance. Recall \blueboxbold{QS13} is an \emph{interleaved querying strategy,} as detailed in Section~\ref{sec:method:simulation:grounding:querying} on page~\pageref{sec:method:simulation:grounding:querying}. What this means is that queries from two other querying strategies (\blueboxbold{QS1} and \blueboxbold{QS3}) were interleaved together. As these two different querying strategies were expected to yield queries of differing performance (poor queries for \blueboxbold{QS1}, good queries for \blueboxbold{QS3}), we needed to make sure that this had been delivered.
...
\begin{table}[t!]
    ...
    \begin{tabulary}{\textwidth}{L{3.67cm}@{\CS}D{3.67cm}@{\CS}D{3.67cm}@{\CS}D{3.67cm}@{\CS}}
        ...
        \RS\lbluecell\textbf{P@1} & \cell 0.03 $\pm$ 0.17 & \cell 0.19 $\pm$ 0.39 & \cell 0.25 $\pm$ 0.43 \\
        \RS\lbluecell\textbf{P@5} & \cell 0.03 $\pm$ 0.13 & \cell 0.15 $\pm$ 0.25 & \cell 0.20 $\pm$ 0.28 \\
        \RS\lbluecell\textbf{P@10} & \cell 0.03 $\pm$ 0.13 & \cell 0.13 $\pm$ 0.21 & \cell 0.16 $\pm$ 0.23 \\
        \RS\lbluecell\textbf{P@20} & \cell 0.03 $\pm$ 0.11 & \cell 0.10 $\pm$ 0.20 & \cell 0.13 $\pm$ 0.19 \\
    \end{tabulary}
    \end{center}
\end{table}

Table~\ref{tbl:ch7_sim_queryperf} provides a series of $P@k$ measures for each of the three individual querying strategies. These values were computed by extracting queries from all 50~\gls{acr:trec} topics trialled for the performance experiments. Queries were extracted from the simulation results, with a total of $1,001$ unique queries identified.

\begin{figure}[t!]
    \centering
    \resizebox{1\hsize}{!}{
    \includegraphics{figures/ch7-queries.pdf}}
    \caption[Example queries issued]{Illustration highlighting several queries issued during the performance \emph{(what-if)} experiments, generated by \blueboxbold{QS13}. The selected topic illustrated is~\gls{acr:trec} topic 448, \emph{ship losses.} Notice the interleaving between single term and three term queries, along with the varying levels of performance, denoted by \emph{P@10}.}
    \label{fig:ch7_example_queries}
\end{figure}

After extracting performance information from the retrieval system and~\gls{acr:trec} QRELs, we then could compute precision -- we report $P@1$, $P@5$, $P@10$ and $P@20$ in the table, along with their respective standard deviations. As expected, we find that queries generated by \blueboxbold{QS1} generally performed poorly, while queries generated by \blueboxbold{QS3} performed substantially better. The performance of the interleaved querying strategy \blueboxbold{QS13} was roughly the average of the other two querying strategies. As such, this provides confirmation that the querying strategies were working as intended, with actual queries (and associated performance values) reported in Figure~\ref{fig:ch7_example_queries}. As we discuss later however, the interleaved querying strategy \blueboxbold{QS13} had an impact upon the performance of several result summary level stopping strategies trialled.
...
what-if runs.
    ...
    - combination strategies do well! SS5 and SS11.
...
\begin{table}[t!]
    \caption[Maximum CG from performance runs]{Results from the simulated what-if simulated performance runs, showing the highest levels of~\gls{acr:cg} attained for each result summary level stopping strategy trialled (grouped by their type). \emph{x\textsubscript{n}} denotes the parameter threshold(s), with \emph{DQ} denoting the depth per query at which the greatest~\gls{acr:cg} value was attained at. These results are presented across the four experimental interfaces. For each interface, the stopping strategy which attained the highest level of~\gls{acr:cg} is \darkbluebox{highlighted}. For the combination stopping strategies, two parameters are presented, with \emph{x\textsubscript{2}/x\textsubscript{4}} presented for \blueboxbold{SS5-COMB} and \emph{x\textsubscript{10}/x\textsubscript{4}} presented for \blueboxbold{SS11-COMB}.}
    ...
    \begin{center}
        \begin{tabulary}{\textwidth}{L{0.00cm}@{\CS}L{1cm}@{\CS}D{0.64cm}@{\CS}D{0.64cm}@{\CS}D{0.64cm}@{\CSONEHALF}D{0.64cm}@{\CS}D{0.64cm}@{\CS}D{0.64cm}@{\CSONEHALF}D{0.64cm}@{\CS}D{0.64cm}@{\CS}D{0.64cm}@{\CSONEHALF}D{0.64cm}@{\CS}D{0.64cm}@{\CS}D{0.64cm}@{\CS}}
            ...
            \RS \multirow{1}{*}{\hspace*{-2mm}\rotatebox{90}{\hspace*{-1mm}\small\textbf{FIX}}} & \lbluecell\small\textbf{SS1} & \dbluecell \small \hspace*{-1mm} 18 & \dbluecell \small \hspace*{-2.5mm} 10.63 & \dbluecell \hspace*{-1mm} \small 2.23 & \dbluecell \small \hspace*{-1mm} 10 & \dbluecell \small \hspace*{-1mm} 6.32 & \dbluecell \hspace*{-1mm} \small 2.14 & \cell \small \hspace*{-1mm} 10 & \cell \small \hspace*{-1mm} 6.41 & \cell \hspace*{-1mm} \small 2.30 & \cell \small \hspace*{-1mm} 10 & \cell \small \hspace*{-1mm} 6.38 & \cell \hspace*{-1mm} \small 2.22 \\

            \RS\RS\RS \multirow{2}{*}{\hspace*{-2mm}\rotatebox{90}{\hspace*{-3mm}\small\textbf{FRUS}}} & \lbluecell\small\textbf{SS2} & \cell \small \hspace*{-1mm} 18 & \cell \small \hspace*{-2.5mm} 12.48 & \cell \hspace*{-1mm} \small 2.13 & \cell \small \hspace*{-1mm} 7 & \cell \small \hspace*{-1mm} 5.99 & \cell \hspace*{-1mm} \small 2.08 & \cell \dbluecell \hspace*{-1mm} 7 & \cell \dbluecell \hspace*{-1mm} 6.13 & \dbluecell \hspace*{-1mm} \small 2.31 & \dbluecell \small \hspace*{-1mm} 7 & \dbluecell \small \hspace*{-1mm} 6.13 & \dbluecell \hspace*{-1mm} \dbluecell 2.26 \\

            \RS & \lbluecell\small\textbf{SS3} & \cell \small \hspace*{-1mm} 9 & \cell \small \hspace*{-2.5mm} 14.11 & \cell \hspace*{-1mm} \small 2.18 & \cell \small \hspace*{-1mm} 4 & \cell \small \hspace*{-1mm} 5.48 & \cell \hspace*{-1mm} \small 1.96 & \cell \small \hspace*{-1mm} 6 & \cell \small \hspace*{-2.5mm} 10.64 & \cell \hspace*{-1mm} \small 2.12 & \cell \small \hspace*{-1mm} 4 & \cell \small \hspace*{-1mm} 5.84 & \cell \hspace*{-1mm} \small 2.12 \\

            \RS\RS\RS \multirow{1}{*}{\hspace*{-2mm}\rotatebox{90}{\small\textbf{SAT}}} & \lbluecell\small\textbf{SS4} & \cell \small \hspace*{-1mm} 3 & \cell \small \hspace*{-2.5mm} 10.07 & \cell \hspace*{-1mm} \small 2.12 & \cell \small \hspace*{-1mm} 2 & \cell \small \hspace*{-1mm} 5.64 & \cell \hspace*{-1mm} \small 2.09 & \cell \small \hspace*{-1mm} 2 & \cell \small \hspace*{-1mm} 5.28 & \cell \hspace*{-1mm} \small 2.25 & \cell \small \hspace*{-1mm} 3 & \cell \small \hspace*{-1mm} 7.05 & \cell \hspace*{-1mm} \small 2.18 \\
            
            \RS\RS\RS \multirow{1}{*}{\hspace*{-2mm}\rotatebox{90}{\small\textbf{COM}}} & \lbluecell\small\textbf{SS5} & \cell \small \hspace*{-2mm} 15/3 & \cell \small \hspace*{-1mm} 8.55 & \cell \hspace*{-1mm} \small 2.19 & \cell \small \hspace*{-2mm} 15/2 & \cell \small \hspace*{-1mm} 5.41 & \cell \hspace*{-1mm} \small 2.12 & \dbluecell \small \hspace*{-1mm} 7/7 & \dbluecell \small \hspace*{-1mm} 6.13 & \dbluecell \hspace*{-1mm} \small 2.31 & \dbluecell \small \hspace*{-1mm} 7/4 & \dbluecell \small \hspace*{-1mm} 5.72 & \dbluecell \hspace*{-1mm} \small 2.26 \\

            \RS\RS\RS \multirow{2}{*}{\hspace*{-2mm}\rotatebox{90}{\small\textbf{DIFF}}} & \lbluecell\small\textbf{SS6} & \cell \small \hspace*{-1mm} 0.80 & \cell \small \hspace*{-2.5mm} 12.86 & \cell \hspace*{-1mm} \small 1.87 & \cell \small \hspace*{-1mm} 0.50 & \cell \small \hspace*{-1mm} 6.34 & \cell \hspace*{-1mm} \small 1.78 & \cell \small \hspace*{-1mm} 0.50 & \cell \small \hspace*{-1mm} 6.36 & \cell \hspace*{-1mm} \small 1.88 & \cell \small \hspace*{-1mm} 0.50 & \cell \small \hspace*{-1mm} 6.31 & \cell \hspace*{-1mm} \small 1.87 \\

            \RS & \lbluecell\small\textbf{SS7} & \cell \small \hspace*{-1mm} 4.00 & \cell \small \hspace*{-1mm} 9.37 & \cell \hspace*{-1mm} \small 1.67 & \cell \small \hspace*{-1mm} 4.50 & \cell \small \hspace*{-1mm} 7.09 & \cell \hspace*{-1mm} \small 1.63 & \cell \small \hspace*{-1mm} 4.50 & \cell \small \hspace*{-1mm} 7.07 & \cell \hspace*{-1mm} \small 1.79 & \cell \small \hspace*{-1mm} 4.50 & \cell \small \hspace*{-1mm} 7.10 & \cell \hspace*{-1mm} \small 1.76 \\

            \RS\RS\RS \multirow{1}{*}{\hspace*{-2mm}\rotatebox{90}{\small\textbf{IFT}}} & \lbluecell\small\textbf{SS8} & \cell \small \hspace*{-2.4mm} 0.002 & \cell \small \hspace*{-2.5mm} 15.08 & \cell \hspace*{-1mm} \small 1.37 & \cell \small \hspace*{-2.4mm} 0.008 & \cell \small \hspace*{-1mm} 3.53 & \cell \hspace*{-1mm} \small 1.71 & \cell \small \hspace*{-2.4mm} 0.010 & \cell \small \hspace*{-1mm} 3.85 & \cell \hspace*{-1mm} \small 1.83 & \cell \small \hspace*{-2.4mm} 0.010 & \cell \small \hspace*{-1mm} 3.54 & \cell \hspace*{-1mm} \small 1.86 \\

            \RS\RS\RS \multirow{2}{*}{\hspace*{-2mm}\rotatebox{90}{\hspace*{-2mm}\small\textbf{TIME}}} & \lbluecell\small\textbf{SS9} & \cell \small \hspace*{-1mm} 60 & \cell \small \hspace*{-1mm} 8.57 & \cell \hspace*{-1mm} \small 2.16 & \cell \small \hspace*{-1mm} 60 & \cell \small \hspace*{-1mm} 5.56 & \cell \hspace*{-1mm} \small 2.12 & \cell \small \hspace*{-1mm} 60 & \cell \small \hspace*{-1mm} 6.14 & \cell \hspace*{-1mm} \small 2.26 & \cell \small \hspace*{-1mm} 60 & \cell \small \hspace*{-1mm} 5.63 & \cell \hspace*{-1mm} \small 2.19 \\

            \RS & \lbluecell\small\textbf{SS10} & \cell \small \hspace*{-1mm} 30 & \cell \small \hspace*{-2.5mm} 11.80 & \cell \hspace*{-1mm} \small 1.92 & \cell \small \hspace*{-1mm} 20 & \cell \small \hspace*{-1mm} 6.44 & \cell \hspace*{-1mm} \small 1.93 & \cell \small \hspace*{-1mm} 20 & \cell \small \hspace*{-1mm} 6.33 & \cell \hspace*{-1mm} \small 2.08 & \cell \small \hspace*{-1mm} 20 & \cell \small \hspace*{-1mm} 5.70 & \cell \hspace*{-1mm} \small 1.95 \\
            
            \RS\RS\RS \multirow{1}{*}{\hspace*{-2mm}\rotatebox{90}{\small\textbf{COM}}} & \lbluecell\small\textbf{SS11} & \cell \small \hspace*{-1mm} 20/5 & \cell \small \hspace*{-1mm} 8.47 & \cell \hspace*{-1mm} \small 2.09 & \cell \small \hspace*{-1mm} 20/4 & \cell \small \hspace*{-1mm} 6.78 & \cell \hspace*{-1mm} \small 2.06 & \cell \small \hspace*{-1mm} 20/4 & \cell \small \hspace*{-1mm} 6.52 & \cell \hspace*{-1mm} \small 2.16 & \cell \small \hspace*{-1mm} 20/4 & \cell \small \hspace*{-1mm} 5.95 & \cell \hspace*{-1mm} \small 2.12 \\
            
            \RS\RS\RS \multirow{1}{*}{\hspace*{-2mm}\rotatebox{90}{\small\textbf{RBP}}} & \lbluecell\small\textbf{SS12} & \cell \small \hspace*{-1mm} 0.99 & \cell \small \hspace*{-1mm} 8.78 & \cell \hspace*{-1mm} \small 2.01 & \cell \small \hspace*{-1mm} 0.95 & \cell \small \hspace*{-1mm} 4.74 & \cell \hspace*{-1mm} \small 1.95 & \cell \small \hspace*{-1mm} 0.99 & \cell \small \hspace*{-1mm} 8.49 & \cell \hspace*{-1mm} \small 2.12 & \cell \small \hspace*{-1mm} 0.99 & \cell \small \hspace*{-1mm} 8.56 & \cell \hspace*{-1mm} \small 2.09 \\
...
\begin{table}[t!]
    ...
    \begin{center}
        \begin{tabulary}{\textwidth}{L{0.00cm}@{\CS}L{2.9cm}@{\CS}D{1cm}@{\CS}D{1cm}@{\CSONEHALF}D{1cm}@{\CS}D{1cm}@{\CSONEHALF}D{1cm}@{\CS}D{1cm}@{\CSONEHALF}D{1cm}@{\CS}D{1cm}@{\CS}}
            ...
            \RS \multirow{1}{*}{\hspace*{-2mm}\rotatebox{90}{\hspace*{-1mm}\small\textbf{FIX}}} & \lbluecell\small\textbf{SS1-FIX} & \cell \small \hspace*{-1mm} 21 & \cell \small \hspace*{-1.5mm} 208.13  & \cell \small \hspace*{-1mm} 18 & \cell \small \hspace*{-1.5mm} 228.04  & \dbluecell \small \hspace*{-1mm} 15 & \dbluecell \small \hspace*{-1.5mm} 185.92  & \cell \small \hspace*{-1mm} 18 & \cell \small \hspace*{-1.5mm} 208.51  \\
            
            \RS\RS\RS \multirow{2}{*}{\hspace*{-2mm}\rotatebox{90}{\hspace*{-3mm}\small\textbf{FRUS}}} & \lbluecell\small\textbf{SS2-NT} & \cell \small \hspace*{-1mm} 18 & \cell \small \hspace*{-1.5mm} 207.51  & \cell \small \hspace*{-1mm} 15 & \cell \small \hspace*{-1.5mm} 230.04  & \cell \small \hspace*{-1mm} 10 & \cell \small \hspace*{-1.5mm} 194.46  & \cell \small \hspace*{-1mm} 10 & \cell \small \hspace*{-1.5mm} 216.17  \\
            ...
            \RS\RS\RS \multirow{1}{*}{\hspace*{-2mm}\rotatebox{90}{\small\textbf{SAT}}} & \lbluecell\small\textbf{SS4-SAT} & \cell \small \hspace*{-1mm} 3 & \cell \small \hspace*{-1.5mm} 210.60  & \cell \small \hspace*{-1mm} 4 & \cell \small \hspace*{-1.5mm} 225.31  & \cell \small \hspace*{-1mm} 4 & \cell \small \hspace*{-1.5mm} 188.15  & \cell \small \hspace*{-1mm} 3 & \cell \small \hspace*{-1.5mm} 209.24  \\
            
            \RS\RS\RS \multirow{1}{*}{\hspace*{-2mm}\rotatebox{90}{\small\textbf{COM}}} & \lbluecell\small\textbf{SS5-COMB} & \dbluecell \small \hspace*{-1mm} 21/4 & \dbluecell \small \hspace*{-1.5mm} 202.94  & \cell \small \hspace*{-1mm} 15/8 & \cell \small \hspace*{-1.5mm} 226.44  & \cell \small \hspace*{-1mm} 21/4 & \cell \small \hspace*{-1.5mm} 186.99  & \cell \small \hspace*{-1mm} 18/4 & \cell \small \hspace*{-1.5mm} 207.11  \\
            
            \RS\RS\RS \multirow{2}{*}{\hspace*{-2mm}\rotatebox{90}{\small\textbf{DIFF}}} & \lbluecell\small\textbf{SS6-DT} & \cell \small \hspace*{-1mm} 0.70 & \cell \small \hspace*{-1.5mm} 279.11  & \cell \small \hspace*{-1mm} 0.60 & \cell \small \hspace*{-1.5mm} 288.38  & \cell \small \hspace*{-1mm} 0.60 & \cell \small \hspace*{-1.5mm} 208.50  & \cell \small \hspace*{-1mm} 0.60 & \cell \small \hspace*{-1.5mm} 234.35  \\
            ...
            \RS\RS\RS \multirow{1}{*}{\hspace*{-2mm}\rotatebox{90}{\small\textbf{IFT}}} & \lbluecell\small\textbf{SS8-IFT} & \cell \small \hspace*{-1.5mm} 0.004 & \cell \small \hspace*{-1.5mm} 269.69  & \cell \small \hspace*{-1.5mm} 0.004 & \cell \small \hspace*{-1.5mm} 257.60  & \cell \small \hspace*{-1.5mm} 0.006 & \cell \small \hspace*{-1.5mm} 219.03  & \cell \small \hspace*{-1.5mm} 0.006 & \cell \small \hspace*{-1.5mm} 251.29  \\
            
            \RS\RS\RS \multirow{2}{*}{\hspace*{-2mm}\rotatebox{90}{\hspace*{-2mm}\small\textbf{TIME}}} & \lbluecell\small\textbf{SS9-TIME} & \cell \small \hspace*{-1mm} 90 & \cell \small \hspace*{-1.5mm} 211.23  & \dbluecell \small \hspace*{-1mm} 120 & \dbluecell \small \hspace*{-1.5mm} 225.06  & \cell \small \hspace*{-1mm} 90 & \cell \small \hspace*{-1.5mm} 186.11  & \cell \small \hspace*{-1mm} 120 & \cell \small \hspace*{-1.5mm} 207.56  \\
            ...
            \RS\RS\RS \multirow{1}{*}{\hspace*{-2mm}\rotatebox{90}{\small\textbf{COM}}} & \lbluecell\small\textbf{SS11-COMB} & \cell \small \hspace*{-1mm} 30/5 & \cell \small \hspace*{-1.5mm} 212.73  & \cell \small \hspace*{-1mm} 40/8 & \cell \small \hspace*{-1.5mm} 243.39  & \cell \small \hspace*{-1mm} 30/9 & \cell \small \hspace*{-1.5mm} 196.96  & \cell \small \hspace*{-1mm} 30/10 & \cell \small \hspace*{-1.5mm} 207.33  \\
            
            \RS\RS\RS \multirow{1}{*}{\hspace*{-2mm}\rotatebox{90}{\small\textbf{RBP}}} & \lbluecell\small\textbf{SS12-RBP} & \cell \small \hspace*{-1mm} 0.99 & \cell \small \hspace*{-1.5mm} 229.84  & \cell \small \hspace*{-1mm} 0.99 & \cell \small \hspace*{-1.5mm} 233.71  & \cell \small \hspace*{-1mm} 0.99 & \cell \small \hspace*{-1.5mm} 192.70  & \cell \small \hspace*{-1mm} 0.99 & \cell \small \hspace*{-1.5mm} 205.82  \\
...
\begin{figure}[t!]
    ...
    \caption[Comparison plots across each stopping strategy and interface]{MSE PLOTS}
...
\begin{table}[t!]
    ...
    \begin{center}
        \begin{tabulary}{\textwidth}{L{0.00cm}@{\CS}L{2.9cm}@{\CS}D{1cm}@{\CS}D{1cm}@{\CSONEHALF}D{1cm}@{\CS}D{1cm}@{\CSONEHALF}D{1cm}@{\CS}D{1cm}@{\CSONEHALF}D{1cm}@{\CS}D{1cm}@{\CS}}
            ...
            \RS & & \lbluecell\small\textbf{CG} & \lbluecell\small\textbf{DQ} & \lbluecell\small\textbf{CG} & \lbluecell\small\textbf{DQ} & \lbluecell\small\textbf{CG} & \lbluecell\small\textbf{DQ} & \lbluecell\small\textbf{CG} & \lbluecell\small\textbf{DQ} \\
            
            \RS & \dbluecell\small\textbf{Real-World} & \cell \small \hspace*{-1mm} xx & \cell \small \hspace*{-1mm} xx & \cell \small \hspace*{-1mm} xx & \cell \small \hspace*{-1mm} xx & \cell \small \hspace*{-1mm} xx & \cell \small \hspace*{-1mm} xx & \cell \small \hspace*{-1mm} xx & \cell \small \hspace*{-1mm} xx \\
            
            \RS\RS\RS \multirow{1}{*}{\hspace*{-2mm}\rotatebox{90}{\hspace*{-1mm}\small\textbf{FIX}}} & \lbluecell\small\textbf{SS1} & \cell \small \hspace*{-1mm} xx & \cell \small \hspace*{-1mm} xx & \cell \small \hspace*{-1mm} xx & \cell \small \hspace*{-1mm} xx & \cell \small \hspace*{-1mm} xx & \cell \small \hspace*{-1mm} xx & \cell \small \hspace*{-1mm} xx & \cell \small \hspace*{-1mm} xx \\
