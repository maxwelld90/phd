...
\begin{itemize}
    ...
    \item{\blueboxbold{\glsfirst{acr:cg}}-based measures, including a widely-used and extensively trialled variant, \blueboxbold{\glsfirst{acr:dcg}};}
    \item{\blueboxbold{\glsfirst{acr:rbp}}, considering a decreasing probability of interaction the further down a list of ranked results; and}
    \item{\blueboxbold{INST}, a more complex user-sided evaluation measure.}
\end{itemize}

We now take each of these measures in turn, discussing the rationale behind each, with a nod towards explaining how each considers the stopping behaviours of searchers.

\subsubsection{Interactive Precision and Recall}
One of the aforementioned assumptions of the~\gls{acr:trec}-style searcher model is that all documents provided in the ranked list of results are considered relevant. As can be seen from more complex searcher models discussed in Section~\ref{sec:ir_background:user:models}, this is not true in reality. During the~\gls{acr:iir} process, searchers examine a number of different result summaries (and their associated documents), making individual judgements as to the relevance of each. Some may not be relevant, and are disregarded by the searcher.
...
To counter these issues, \emph{interactive precision (iP), interactive recall} and \emph{interactive~\gls{acr:trec} precision} were defined by~\cite{veerasamy1996iir} and~\cite{veerasamy1996iir}. Instead of purely considering precision and recall as measures exclusively utilising the relevance assessments provided as part of a test collection, one would also consider the number of documents considered relevant by a subject of an~\gls{acr:iir} study that were also \blueboxbold{\gls{acr:trec} relevant}. This therefore means that a document judged relevant by an assessor may not be retrieved, viewed and subsequently judged by the subject of an~\gls{acr:iir} study. We demonstrate this in Figure~\ref{fig:interactive_precision}. Assessors provide judgements for documents, with these relevance judgements comprising the \emph{gold standard} to which searcher assessments are compared against. These can differ: in the example illustration, the retrieval system's $P@5$ score is $3/5=0.6$, while the searcher's $iP@5 = 2/5 = 0.4$. This is because although the searcher saved three documents as relevant, the document at rank two was not assessed as such; therefore, only two of the three saved documents count.

\begin{figure}[t!]
    \centering
    \resizebox{1\hsize}{!}{
    \includegraphics{figures/ch2-interactive-precision.pdf}}
    \caption[Interactive precision example]{A visual example of \emph{interactive precision} alongside traditional, system-sided precision. In the example ranking, the retrieval system returned three relevant documents, yet the searcher only correctly identified two of them, hence an interactive precision \emph{(iP)} score of \emph{2/5=0.4.}}
