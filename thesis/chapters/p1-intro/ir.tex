...
A simple approach that employs the~\gls{acr:prp} is the \emph{Binary Independence Model (BIM).} Simple assumptions that make the implementation of the~\gls{acr:prp} straightforward are employed. As the name of the model suggests, one such assumption is the notion of binary relevance in term vectors for a given document (i.e. a term either exists in a document or not). Independence refers to the assumption that terms are modelled as occurring in documents independently, with no association between terms~\citep{manning2008ir}.

first part from manning links BIM to BM25

say that the BIM is a simple model using the PRP.
state the assumption.
them move on to bm25.
