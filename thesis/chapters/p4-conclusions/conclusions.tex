...
ch8 searchers more efficient than ch7 searchers, hypothesise that the goals varies and influences your decisions/ability to search.
    ...
    \begin{table}[t!]
        \caption[GAIN]{Summarising details across the eight interfaces and conditions trialled. Included in this table is the reported average session time (\genericblack{Avg. S.T.}), mean \genericblack{\gls{acr:cg}} attained, and the cost per unit of~\gls{acr:cg}, or \genericblack{Cost/Unit~\gls{acr:cg}}. All $\pm$ values denote the standard deviations. Values reported are over the real-world subjects of the two user studies conducted, as reported in Chapters~\ref{chap:snippets} and~\ref{chap:diversity}.}
