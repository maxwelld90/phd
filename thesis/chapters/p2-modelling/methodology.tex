...
Each of the stopping strategies, along with threshold values trialled, are enumerated below.
...
Having now discussed how all of the various components of the~\gls{acr:csm} and the \simiir~framework were instantiated for our simulations, we now move to a discussion of how we actually ran the simulations. We also provide a brief discussion of our evaluation approach. The simulations runs were based upon the two research questions posed back in Section~\ref{sec:intro:rqs} on page~\pageref{sec:intro:rqs}. As such, we considered two different sets of runs.

\begin{itemize}
    \item[]{\blueboxbold{HL-RQ3a} \emph{Given the operationalised stopping strategies outlined in Chapter~\ref{chap:strategies}, how well do simulated searchers using these strategies perform?} To address this question, we propose a series of \blueboxbold{performance runs} that allow us to determine the best overall level of performance that can be attained using a particular configuration of simulated searcher.}
    \item[]{\blueboxbold{HL-RQ3b} \emph{How closely do the operationalised stopping strategies compare to the stopping behaviours of real-world searchers?} To address this second research question, we also propose a series of \blueboxbold{comparison runs} that instead focus upon how closely different configurations of simulated searcher approximate the stopping depth of real-world searchers.}
\end{itemize}
...
\begin{figure}[t!]
    ...
    \caption[Sim Evaluation]{Evaluation Framework}
    \label{fig:sim_evaluation}
\end{figure}

- simulations fit into the broader framework as shown in Figure~\ref{}.
- of note is the generation of an interaction log for each simulation configuration trialled, with a series of scripts that then convert this to a summary which we can then analyse and report.
- the figure also provides a visual illustration of the difference between the two sets of runs, which we discuss in detail below.

\subsubsection{Performance Runs}

- examine the overall performance of each configuration.
- 
...
Rather than focus on the overall performance attained by simulated searchers, this set of simulations focuses instead on making a fair comparison between the simulated searcher and real-world counterparts. As such, we follow the same approach as the general methodology explained above, except for two key differences, which:

\begin{itemize}
    
    \item{consider only the \blueboxbold{four topics} used within the user studies; and}
    \item{\blueboxbold{replaying the queries} issued by the real-world subjects, rather than generating them through a querying strategy.}
    
\end{itemize}

we do the same, but this time only over four topics, and replaying the different

\blueboxheader{Comparing Simulated Searchers and Real-World Searchers}
As discussed previously, comparisons are made between the \emph{average} behaviours exhibited by the two, rather than on a per-searcher basis. This average is considered across each interface and condition trialled over the two user studies.
