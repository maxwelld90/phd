...
\begin{figure}[t!]
    ...
    \caption[The problem of stochastic judgements]{An example of the same~\gls{acr:trec} relevant document being judged differently across multiple trials. As discussed in Section~\ref{sec:method:simulation:grounding:judgements}, this can have a negative effect upon simulation runs, yielding dubious results.}
...
\begin{table}[t!]
    ...
    \RS \multicolumn{4}{@{\hskip 6pt}c@{\CSDOUBLE}}{\textbf{Without pre-rolled judgements (Unfair)}} & \multicolumn{4}{@{\hskip 0pt}c}{\textbf{With pre-rolled judgements (Fair)}}\\
...
In Figure~\ref{fig:multiple_runs}, if the same document is judged differently between individual trials, then comparisons between different simulated configurations becomes fundamentally unfair. This is demonstrated in the left hand side of Table~\ref{tbl:precomputed_judgements}. With a given probability, a result summary can be judged to be attractive or not ($P(C)$). With proper controls, these judgements will vary across trials, meaning that in the table above, a comparison between configuration \dualbluebox{SS1}{@5} and \dualbluebox{SS2}{@2} is unfair (refer to Section~\ref{sec:method:simulation:grounding:stopping} for more information on these configurations), because they differ. As the judgements change, this unfairly changes the behaviour of the simulated searcher across the configurations.
...
Next, we enumerate the stopping strategy considering the satisfaction, or \emph{satiation}~\citep{simon1955satiation} of a searcher.
...
\begin{itemize}
    \item{\blueboxbold{SS8-OFT} Computing the searcher's average rate of gain as defined above, we considered a gain threshold ($x_8$) from $0.002$ to $0.03$ in steps of $0.002$. A minimum of two result summaries were examined before calculating the average rate of gain ($y_8=2$).}
\end{itemize}
...
The second combination rule, based upon the combination heuristic proposed by~\cite{mcnair1982gut_mvt}, considers that a searcher has some notion as to whether a given~\gls{acr:serp} is of a high or low scent. Depending upon the scent observed, a different stopping strategy will be employed.
...
\begin{table}[t!]
    ...
    \begin{center}
    \vspace*{-2mm}
    \begin{tabulary}{\textwidth}{D{4cm}@{\CS}L{11.8cm}}
...
\subsubsection{Simulated Searcher Constraints}\label{sec:method:simulation:grounding:constraints}
What next?
