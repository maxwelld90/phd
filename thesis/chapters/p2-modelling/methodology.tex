...
\todo{Mention that this comes from a scent background.}
\todo{The scent influences your behaviour.}
\todo{Talk about the definitoion of what a poor scent is.}
\todo{Then talk about the probabilities, and how we extracted them.}
\todo{Then draw the diagram explaining the probabilities.}
\todo{From there, discuss the three strategies.}
\todo{}

Previously outlined in Section~\ref{sec:csm:new_stopping}, the~\gls{acr:csm} includes a~\gls{acr:serp} level stopping decision point. This is in addition to the two established snippet level and session level stopping decision points. The additional stopping decision point allows for a searcher subscribing to the~\gls{acr:csm} to either begin examining individual result summaries for attractiveness, or abandon the presented~\gls{acr:serp} in its entirety. In this section, we provide details on how we operationalised this new stopping decision point, considering: the definitions we used for implementation; a discussion of a new set of interaction probabilities, used specifically for this stopping decision point; and a discussion of the three different approaches we implemented.
...
\begin{wrapfigure}[12]{r}{0.45\textwidth}
    ...
    \end{center}
    \vspace*{-4mm}
    \caption[Viewport cutoff example]{The~\gls{acr:serp} viewport threshold. In this example, three result summaries are visible, with two not visible within the viewport. Therefore, \emph{view\textsubscript{size}=3}. Cheers!}
    \label{fig:viewport_cutoff}
\end{wrapfigure}

\blueboxheader{Considering a \glsplural{acr:serp} Scent} Before we can discuss how we implemented the new stopping decision point, we must first clarify the definition we used for determining what we considered a poor quality~\gls{acr:serp}. If we consider a~\gls{acr:serp} from the perspective of~\glsfirst{acr:ift} (refer to Section~\ref{sec:stopping_background:models:theoretical:ift}), considering said~\gls{acr:serp} as a \emph{patch}, we can then consider the \emph{information scent} of the patch. For a given~\gls{acr:serp}, if $P@10=0.0$, the scent offered would be considered low. This value was suggested by~\cite{wu2014information_scent}, who stated that a~\gls{acr:serp} that returns little or no relevant content can be considered to offer a low information scent. We refer to this value as the \emph{patch threshold.}

\blueboxheader{The Probability of Examining a~\gls{acr:serp}}
These probabilities are interface/condition dependent, so we do not present them here. However, refer to the relevant Chapter for more information.

\blueboxheader{Three~\gls{acr:serp} Stopping Implementations}
We can now outline the three different strategies that we trialled for the new~\gls{acr:serp} level stopping decision point. These are enumerated below, with our baseline implementation listed first.

\begin{itemize}
    \item{\blueboxbold{Always Examine (Baseline)} With this strategy, a searcher will \emph{always} enter a given~\gls{acr:serp} and examine at least one result summary (dependent upon the snippet level stopping strategy selected). This is considered as the current state-of-the-art.}
    \item{\blueboxbold{Perfect~\gls{acr:serp} Judgements} Here, a simulated searcher will only begin to examine a~\gls{acr:serp} in detail if $P@k > 0$. If $P@k = 0$, the searcher will abandon the~\gls{acr:serp}, and then proceed to the next action as dictated by the~\gls{acr:csm}. This can be considered as the upper bound in terms of performance -- analogous to, as an example, the \emph{ideal user} of~\cite{hagen2016simulating_users}.}
    \item{\blueboxbold{Stochastic~\gls{acr:serp} Judgements} Our third strategy uses a stochastic element to determine whether the simulated searcher should enter the~\gls{acr:serp} or not. Like above, the $P@k$ of the~\gls{acr:serp} is computed. If the~\gls{acr:serp} is deemed to be of high scent, }
...
extract this from the ecir paper.

\subsubsection{Snippet Level Stopping Strategies}\label{chap:csm:method:simulation:grounding:stopping_strats}
enumerate the different stopping strategies.
add explanation as to where values come from, based upon what Leif said.
...
two research questions: how do we do this?

note that we have an interaction log for each simulation configuration trialled, similar to the interaction log shown in the previous section.

\subsubsection{Performance Runs}
mention that performance runs discuss the stuff above, over the 50 topics.
queries generated via the querying strategies.

\subsubsection{Comparison Runs}
we do the same, but this time only over four topics, and replaying the different
