...
\begin{table}[t!]
    \caption[Example of re-rolled judgements]{A visual example of how the use of pre-rolled judgements (right) performs when compared without (left). Across two sample simulated searcher configurations, judgements on the left table vary compared to those on the right table. As such, this can have an undue influence upon the behaviour of a simulated searcher, such as affecting their stopping behaviour. This is illustrated below, with the stopping depth for \dualbluebox{SS2}{@2} being affected (stopping depths highlighted in red).}
