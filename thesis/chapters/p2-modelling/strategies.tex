...
\begin{itemize}
    \item{\dualbluebox{SS11-COMB}{Combination – Time and satiation} Encountering a~\gls{acr:serp} expected to yield a high volume of relevant content early on (high scent), a searcher will employ the satisfaction/satiation stopping heuristic. However, if the~\gls{acr:serp} is judged to yield relevant items over greater depths or is judged to be of poor quality (low scent), the give-up time-based heuristic is used instead.}
\end{itemize}
