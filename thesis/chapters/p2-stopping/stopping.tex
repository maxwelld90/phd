...
A well known conceptual model is the so-called \emph{Berry Picking model} proposed by~\cite{bates1989berry_picking}. In this model, an analogy is struck between a searcher and a \emph{forager} -- a forager scavenges between different bushes, looking to collect the juiciest and ripest berries, as depicted in Figure~\ref{fig:berry_picking}. This is likened to a searcher examining a number of different~\glsplural{acr:serp}, where the searcher looks for the documents that are most relevant to his or her information need (or the juiciest berries). While this model is intuitive, there is no explanation for how long an individual should spend examining results, or how the results/berries would affect the searcher's behaviour. However, it is alluded to later by~\cite{bates1989alluding} that searchers could weigh up the costs and benefits in order to decide what to do next.
...
Following on from the notion of searchers being analogous 

What is IFT?
What is the motivation behind it?

Now, what is the theory comprised of? There are three underlying models here.
Scent model, and patch model.

What does the scent model tell us?
    - We use this later on in the thesis.

What does the patch model tell us?
    - Illustration from the ECIR slides.
    - Also provides an explanation of when people stop.
