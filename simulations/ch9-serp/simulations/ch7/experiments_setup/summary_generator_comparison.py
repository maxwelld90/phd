...
def get_log_data(simulation_output_path, base_filename, qrels_handler, topic):
    """
    ...
    for line in f:
        ...
        if action == 'QUERY':
            query_details.append({'terms': ' '.join(line[4:]), 'hover_depth': 0, 'click_depth': 0, 'cg': 0, 'saved_trec_rel': 0})
...
def get_simulation_output_components(simulation_output_path, qrels_handler):
    """
    ...
    for filename_noextension in no_extension_list:
        ...
        if filename_noextension not in observed_users:
            ...
            log_data = get_log_data(simulation_output_path, filename_noextension, qrels_handler, topic)
...
def generate_combination_output(path_components, combination):
    ...
    for query in combination['query_data']:
        ...
        return_str = '{return_str}{userid},{queryid},{topic},{snip_len},{se},{ser},{ss},{ssr},{decision_maker},{decision_maker_run},{u_n},{u_d},{u_r},{u_t},{u_g},{u_p},{click_depth},{hover_depth},{cg},{saved_trec_rel}{linesep}'.format(
            return_str=return_str,
            userid=path_components['userid'],
            queryid=query['queryid'],
            topic=combination['topic'],
            snip_len=path_components['snip_len'],
...
def main(base_directory, rw_queries_path, qrels_handler):
    """
    Does the magic.
    """
    ...
    order_str = 'userid,queryid,topic,snip_len,se,ser,ss,ssr,decision_maker,decision_maker_run,u_n,u_d,u_r,u_t,u_g,u_p,click_depth,hover_depth,cg,saved_trec_rel'
    print order_str
    ...
    for config_file_path in config_list:
        ...
        experiment_components = get_simulation_output_components(experiment_output_path, qrels_handler)
...
if __name__ == '__main__':
    ...
    main(base_directory, rw_queries_path, qrels_handler)