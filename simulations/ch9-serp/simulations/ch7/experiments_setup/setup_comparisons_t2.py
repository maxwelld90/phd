...
snippet_decision_makers = {
                          'stochastic':       {'className': 'StochasticInformedTrecTextClassifier',
                                               'attributes': [{'name': 'qrel_file', 'type': 'string', 'value': '{aux_dir}/prerolled/t{{prerolledID}}-{{dmRun}}-click.qrels', 'is_argument': 'true'}, 
...
document_decision_makers = {
                          'stochastic':       {'className': 'StochasticInformedTrecTextClassifier',
                                               'attributes': [{'name': 'qrel_file', 'type': 'string', 'value': '{aux_dir}/prerolled/t{{prerolledID}}-{{dmRun}}-mark.qrels', 'is_argument': 'true'}, 
                       ...
                       'QSREAL': {'id': 0,
                                'className': 'PredeterminedQueryGenerator',
                                 'attributes': [{'name': 'stopword_file', 'type': 'string', 'value': '{aux_dir}/terms/stopwords.txt', 'is_argument': 'true'},
                                                {'name': 'query_file', 'type': 'string', 'value': '{aux_dir}/queries-t{{prerolledID}}.csv', 'is_argument': 'true'},
    ...
    for userid in userids:
        ...
        for interface in interfaces:
            ...
            for serp_impression in serp_impressions:
                ...
                if interface not in ['t0', 't1', 't2', 't4'] and serp_impression != 'stochastic':
                    continue
                                ...
                                for dm in stopping_strategies[ss]['decision_makers']:
                                    ...
                                    for j in range(0, dm_runs):
                                        ...
                                        for querying_strategy in stopping_strategies[ss]['querying']:
                                            base_path = "/u{userid}/t{interface}/se{serp_impression}/ser{serp_impression_run:02d}/ti{timeout}/ss{ss}/ssr{ssr}/dm{dm}/dmr{dmr:02d}/qs{qs}/".format(
                                                userid=userid,
                                            ...
                                            experiment_id = "exp-u{userid}_t{interface}_se{serp_impression}_ser{serp_impression_run:02d}_ti{timeout}_ss{ss}_ssr{ssr}_dm{dm}_dmr{dmr:02d}_qs{qs}".format(
                                                userid=userid,
                                                interface=interfaces[interface]['id'],
                                                serp_impression=serp_impressions[serp_impression]['id'],
                                                serp_impression_run=i,
...
def create_simulation_file(experiment_id, experiment_path, index_dir, frag_type, frag_size, frag_surround, users, aux_dir, queries_file, userid):
