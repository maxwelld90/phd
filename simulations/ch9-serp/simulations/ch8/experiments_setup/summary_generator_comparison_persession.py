...
SIM_OUTPUT_FIELDS = {
    ...
    'calculatable': [
        ...
        'depth_per_query',
        'marked_trec_rel',
        'docs_with_new_entities',
        'click_depth_per_query',
    ],
}
...
def get_simulation_output_components(simulation_output_path, qrel_handler, diversity_qrel_handler):
    """
    ...
    # Added for debugging!
    if '.DS_Store' in all_output_files:
        all_output_files.pop(all_output_files.index('.DS_Store'))
    ...
    for filename_noextension in no_extension_list:
        ...
        if filename_noextension not in observed_users:
            ...
            calculated['docs_with_new_entities'] = utils.calculate_new_docs_with_entities(qrel_handler, diversity_qrel_handler, topic, filename_noextension_path)
            calculated['new_entities_found'] = utils.calculate_new_entities_found(qrel_handler, diversity_qrel_handler, topic, filename_noextension_path)
            calculated['click_depth_per_query'] = utils.calculate_click_depth_per_query(filename_noextension_path)
...
def get_order_lists(qrel_handler, diversity_qrel_handler, config_list, expected_path_parameters):
    ...
    for config_file_path in config_list:
        ...
        experiment_components = get_simulation_output_components(experiment_output_path, qrel_handler, diversity_qrel_handler)
...
def main(base_directory, qrels_path, eval_diversity_qrels_path):
    """
    Does the magic.
    """
    ...
    diversity_qrel_handler = EntityQrelHandler(eval_diversity_qrels_path)
    
    # Inefficent, but I do this twice to get the list of parameters that are used to construct an output file.
    orders = get_order_lists(qrel_handler, diversity_qrel_handler, config_list, expected_path_parameters)
...
if __name__ == '__main__':
    ...
    eval_diversity_qrels_path = sys.argv[3]
    main(base_directory, qrels_path, eval_diversity_qrels_path)
