...
interfaces = {
              ...
              #        'snip_size': 2,
              #
              #        'interface': 'WhooshDiversifiedInterface',
              #        'diversity_qrels': '{aux_dir}/combined.diversity.qrels',
              #        'lam': 0.7,
              #        'to_rank': 30,
              #
              #        'pcr': 0.27,
...
decision_makers = {'stochastic': {'id': 1, 'runs': 50}}
...
logger = {'className': 'FixedCostGoalLogger',
          'attributes': [{'name': 'time_limit', 'type': 'float', 'value': '{{timeLimit}}', 'is_argument': 'true'},
                         {'name': 'marked_goal', 'type': 'integer', 'value': '6', 'is_argument': 'true'},
                            ...
                            for dm in stopping_strategies[ss]['decision_makers']:
                                ...
                                for j in range(0, dm_runs):
                                    ...
                                    for querying_strategy in stopping_strategies[ss]['querying']:
                                        ...
                                        base_path = "/i{interface}/se{serp_impression}/ser{serp_impression_run:02d}/ti{timeout}/ss{ss}/ssr{ssr}/dm{dm}/dmr{dmr:02d}/qs{qs}/".format(
                                        ...
                                        experiment_id = "exp-i{interface}_se{serp_impression}_ser{serp_impression_run:02d}_ti{timeout}_ss{ss}_ssr{ssr}_dm{dm}_dmr{dmr:02d}_qs{qs}".format(
                                            interface=interfaces[interface]['id'],
                                            serp_impression=serp_impressions[serp_impression]['id'],
                                            serp_impression_run=i,
                                            timeout=timeout,
...
def create_simulation_file(experiment_id, experiment_path, index_dir, frag_type, frag_size, frag_surround, interface, diversity_qrels, lam, to_rank, users, aux_dir):
