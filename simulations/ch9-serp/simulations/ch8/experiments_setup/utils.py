...
def calculate_click_depth_per_query(base_filename):
    """
    Returns the mean click depth over all queries issued.
    """
    log_filename = '{0}.log'.format(base_filename)
    log_file = open(log_filename, 'r')
    ...
    if len(click_depths) == 0:
        return 0.0
    
    return sum(click_depths) / float(len(click_depths))


def calculate_new_docs_with_entities(qrel_handler, diversity_qrel_handler, topic, base_filename):
    """
    Given a base filename, returns the number of documents that have been saved with at least one new entity.
    """
    log_filename = '{0}.log'.format(base_filename)
    log_file = open(log_filename, 'r')
    observed_entities = []  # What entities have been observed?
    document_count = 0
    ...
    for line in log_file:
        ...
        if action == 'MARK' and line[4] == 'CONSIDERED_RELEVANT':
            docid = line[5]
            entities_in_document = diversity_qrel_handler.get_mentioned_entities_for_doc(topic, docid)
            
            for entity in entities_in_document:
                if entity not in observed_entities:
                    observed_entities.append(entity)
                    document_count = document_count + 1
                    break  # Stop once a new entity has been observed; the document count has been increased.
    
    
    log_file.close()
    return document_count

def calculate_new_entities_found(qrel_handler, diversity_qrel_handler, topic, base_filename):
    """
    Given a base filename, returns the number of entities that have been found in the documents saved.
    """
    log_filename = '{0}.log'.format(base_filename)
    log_file = open(log_filename, 'r')
    observed_entities = []  # What entities have been observed?
